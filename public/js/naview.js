...
class NaView {
    ...
    getTextRules() {
        this.text_to_draw = this.deepCopy(this.parsed_protein_data.draw_symbols);
        return this.deepCopy(this.text_to_draw);
    }
    ...
    *    2a. A skew value CAN added to the x positioning of this centroid point<br>
    ...
    draw_residue_relations(residue_relation_data, centroid_data, full_protein_data) {
        ...
        if (draw_opts.path_width_scaling.type === "calc") {
            ...
            current_width_scale = function(etc) {
                return that.svg_width * draw_opts.path_width_scaling.perc_x;
            };
        } else {
            current_width_scale = function(etc) {
                return 0;
        ...
        console.log("draw_opts.color_scaling");
        console.log(draw_opts.color_scaling);
        if (draw_opts.color_scaling.type === "weight") {
            if (draw_opts.color_scaling.property === "weight") {
                ...
                if (!draw_opts.color_scaling.range) {
                    draw_opts.color_scaling.range = ["lightblue", "blue"];
                }
        ...
        } else if (draw_opts.color_scaling.type === "type") {
            current_color_scale_prop = draw_opts.color_scaling.type
            let cdomain = draw_opts.color_scaling.domain;
            let crange = draw_opts.color_scaling.range;
            // current_color_scale= d3.scaleLinear()
            // .domain(cdomain)
    ...
    draw_text_symbols(symbols_data) {
        ...
        function draggedText(d, i) {
            ...
            .attr('y', my);
            updateTextBox(i);
            updateTextData(i, ["positioning", "x", "y"], [mx, my]);
            // updateTextData(i, ["positioning", "x", "y"], [mx, my]);
            // updateTextData(i, ["positioning", "y"], my);
            // {type: "absolute", x: 100, y: 100}
            // current_border
            // .attr('x', mx)
            // .attr('y', function() {
        ...
        function updateTextData(tindex, attr_to_up, value_to_up) {
            ...
            if (attr_to_up instanceof Array) {
                if (attr_to_up[0] === "positioning") {
                    // {type: "absolute", x: 100, y: 100}
                    ol_data["positioning"]["type"] = "absolute";
                    ol_data["positioning"][attr_to_up[1]] = value_to_up[0];
                    ol_data["positioning"][attr_to_up[2]] = value_to_up[1];

                    that.parsed_protein_data.draw_symbols[tindex]["positioning"]["type"] = "absolute";
                    that.parsed_protein_data.draw_symbols[tindex]["positioning"][attr_to_up[1]] = value_to_up[0];
                    that.parsed_protein_data.draw_symbols[tindex]["positioning"][attr_to_up[2]] = value_to_up[1];
                }
            } else { //getTextRules
